...
/**
 * MusicRecording for a bhajan/song page — the markup Google understands for a
 * recorded song. Links to the artist (Person), the YouTube video, and (when
 * present) a MusicComposition carrying the lyrics. This is what makes a specific
 * bhajan/song page eligible for song & lyrics-style results.
 */
export function musicRecordingSchema(bhajan = {}) {
  const url = `${siteConfig.url}/bhajans/${bhajan.slug}`;
  const thumb =
    bhajan.thumbnail?.url ||
    (bhajan.youtubeUrl
      ? (() => {
          const m = bhajan.youtubeUrl.match(/(?:youtu\.be\/|[?&]v=|\/embed\/|\/shorts\/)([A-Za-z0-9_-]{11})/);
  ...
  return {
    ...
    url,
    inLanguage: "hi",
    genre: bhajan.genre || "Bhajan",
    byArtist: { "@type": "MusicGroup", "@id": `${siteConfig.url}/#person`, name: siteConfig.name, url: siteConfig.url },
    ...(thumb ? { image: thumb } : {}),
    ...(bhajan.description ? { description: bhajan.description } : {}),
    ...(bhajan.youtubeUrl
      ? { video: videoObjectSchema({ name: bhajan.title, description: bhajan.description, thumbnailUrl: bhajan.thumbnail?.url, uploadDate: bhajan.createdAt, embedUrl: bhajan.youtubeUrl, views: bhajan.views }) }
      : {}),
    ...(bhajan.lyrics
      ? {
          recordingOf: {
            ...
            name: bhajan.title,
            inLanguage: "hi",
            lyricist: { "@type": "Person", name: siteConfig.name },
